...
router.post("/", async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    if (copying) {
      console.log("copying:", originalTaskId)
      let originalTask = await Task.findById(originalTaskId)
        .select(`${labels} ${members} ${checklist} ${attachments} ${comments}`)
        .lean()
      if (!originalTask) {
        return res.status(404).json({ error: "originalTask not found" })
      }
      console.log("originalTask",originalTask)
      // res.status(404).json(originalTask) 
      // console.log(
      //   "Task.create",
      //   originalTask,
      //   list.taskListBoard,
      ...
      //   position
      // )
      const { _id, ...taskData } = originalTask;
        const copiedTask = await Task.create({
        ...taskData,
      ...
      // getIO().to(`board_${task.board}`).emit("taskCreated", task)
      res.status(201).json(copiedTask)
      // res.status(201).json({  board: list.taskListBoard,
      //   list: listId,
      //   position: position,
      //   ...originalTask,})
    } else {
...
router.put("/:id", async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    await Activity.create({
      board: task.board,
      user:
        req.user?.id || new mongoose.Types.ObjectId("000000000000000000000000"),
      entity: { kind: "task", id: task._id },
      action: "updated_task",
      payload: updates,
    })
    getIO().to(`task_${task!._id}`).emit("taskUpdated", task)
